...
test_that("S4 classes are defined correctly", {
  ...
  expect_equal(db_seq@file_source, "test.duckdb")
})
...
test_that("Constructor functions work", {
  ...
  expect_equal(db_file@path, "test.duckdb")
...
test_that("Show methods work without errors", {
  ...
  expect_output(show(db_file), "DuckDBFile")
  expect_output(show(db_seq), "dbSequence")
})
